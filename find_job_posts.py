def find_job_posts(posts, keywords_file):
    ...
    from define_post_city import define_city, define_if_city_equal_filter 
    ...
    for row in posts:
        if define_if_job(row['post'].lower()) and define_if_city_equal_filter(row['post'].lower()):
            city = define_city(row['post'].lower())
            if define_if_city_equal_filter(city):
                row['is_job'] = True
                row['city'] = define_city(row) # присваивает город к посту с вакансией
